...
【核心生成原则：旁白 beat 驱动】
本项目不是让视频模型自由演剧情，而是“旁白漫剧 / 动态漫画”模式。
必须先把整集拆成 narration_beats：一句旁白 = 一个画面任务 = 一个视频任务 = 一个合成片段。
每个 beat 的画面只服务这一句旁白，不能画面讲 A、旁白讲 B。
narration_beats.length 必须等于 ${input.sceneCount}。
scenes.length 也必须等于 ${input.sceneCount}，并且 scenes[i] 必须和 narration_beats[i] 一一对应。
每个 scene.scene_id 必须等于对应 narration_beat.scene_id。
每个 scene.tts[0].text 必须等于对应 narration_beat.narration。
每个 scene.image_prompt 必须直接表现对应 narration_beat.visual_must_show。
每个 scene.video_prompt 只能做对应 narration_beat 的轻微漫画动效，不要新增剧情。
