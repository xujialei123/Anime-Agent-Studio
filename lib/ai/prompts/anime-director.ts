...
从 scene_002 开始，video_prompt 必须包含一句 “Continue the character identity and visual style from previous shot...” 并说明它如何承接上一幕的情绪或位置。
...
【配音规则】
本项目采用“旁白漫剧”模式，不做角色口型同步。每个 scene.tts 必须且只能有一项旁白。
旁白项的 speaker 必须是“旁白”，type 必须是 narrator；不要生成 character 或 inner_voice 类型。
把原本需要角色说出的对白改写成第三人称叙述、转述或内心状态描述，不要让角色直接开口说台词。
每项旁白必须包含 speaker、type、voiceType、emotion、text、speed、volume、voiceDesignPrompt。
所有 scene 使用一致的 voiceType 和 voiceDesignPrompt，保持整集旁白音色统一。
旁白要短、有冲击力，朗读时长不得超过当前 scene.duration_seconds，并在结尾预留约 0.3 秒静音。
video_prompt 不要描述说话、张嘴、对白或口型动作；角色只做与旁白内容对应的表情、姿态和环境动作。
